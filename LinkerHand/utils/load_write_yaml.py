...
class LoadWriteYaml():
    ...
    def delete_action_from_yaml(self, action_name, hand_joint="", hand_type=""):
        """从YAML文件中删除指定动作"""
        if hand_joint == "L20":
            action_path = self.l20_positions
        elif hand_joint == "L10":
        ...
        try:
            ...
            # 删除指定动作
            if hand_type == "left" and yaml_data.get("LEFT_HAND"):
                yaml_data["LEFT_HAND"] = [action for action in yaml_data["LEFT_HAND"] 
                                        if action.get("ACTION_NAME") != action_name]
            elif hand_type == "right" and yaml_data.get("RIGHT_HAND"):
                yaml_data["RIGHT_HAND"] = [action for action in yaml_data["RIGHT_HAND"] 
                                         if action.get("ACTION_NAME") != action_name]
            ...
            print(f"Error deleting action from yaml file: {e}")
            return False

    def edit_action_in_yaml(self, old_name, new_name, hand_joint="", hand_type=""):
        """在YAML文件中编辑动作名称"""
        if hand_joint == "L20":
            action_path = self.l20_positions
        elif hand_joint == "L10":
        ...
        try:
            ...
            # 编辑指定动作名称
            if hand_type == "left" and yaml_data.get("LEFT_HAND"):
                for action in yaml_data["LEFT_HAND"]:
                    if action.get("ACTION_NAME") == old_name:
                        action["ACTION_NAME"] = new_name
                        break
            elif hand_type == "right" and yaml_data.get("RIGHT_HAND"):
                for action in yaml_data["RIGHT_HAND"]:
                    if action.get("ACTION_NAME") == old_name:
                        action["ACTION_NAME"] = new_name
                        break
            ...
            print(f"Error editing action in yaml file: {e}")
            return False
